...
describe('getExtractedSVG()', function() {
   ...
   test('should remove width and height from <svg /> element', function() {
   ...
   it('should apply prefixes to ids', function() {
      ...
      // // replaces xlink:href=
      assert.isTrue(
         processedStyleInsertedSVG.match(/xlink:href=/g).length === 1
      );
      // // replaces url(#foo)
      assert.isTrue(
         processedStyleInsertedSVG.match(/url\(#test\.prefix-foo\)/g).length ===
            1
      );
   });
